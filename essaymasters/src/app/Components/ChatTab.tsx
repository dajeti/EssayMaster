...
export default function ChatTab({ essay }: { essay: string }) {
  ...
  // Modified detection system - more focused on explicit essay writing requests
  const blockedPatterns = [
    // Core essay writing patterns - reduced and more specific
    /\b(write|create)\s+my\s+(entire|full|complete)\s+(essay|paper|assignment)\b/gi,
    /\b(do|complete)\s+my\s+(homework|essay|assignment)\b/gi,
    /\bwrite\s+.*\s+pages?\s+essay\b/gi,
    
    // File format patterns - kept only the most explicit
    /\bgenerating?\s+(a\s+)?(PDF|Word\s+document|docx?)\b/gi
  ];

  const containsBlockedRequest = (text: string): boolean => {
    // Only check if input is longer than a certain threshold to avoid false positives
  ...
  async function handleChatSubmit() {
    ...
    // Update chat history first for better responsiveness
    setChatMessages(prev => [...prev, { sender: "user", text: input }]);
    setIsChatLoading(true);

    try {
      // Check if input contains a blocked pattern after adding to chat
      if (containsBlockedRequest(input)) {
        setChatMessages(prev => [...prev, {
          sender: "ai",
          text: "I can help analyze your existing essay, but I can't write a new one for you. Would you like feedback on a specific part of your essay instead?"
        }]);
        setChatInput("");
        setIsChatLoading(false);
        return;
      ...
      // Adjusted prompt to be more conversational
      const chatPrompt = `
        You are an AI writing tutor assisting with essay analysis. The user provided this essay:
        """${essay}"""

        YOUR GUIDELINES:
        1. Focus on providing helpful feedback on the essay
        2. If the question is about the essay, provide relevant analysis
        3. If the question is not about the essay, you can provide a brief, helpful response
        4. Keep responses concise - around 2-3 paragraphs when possible
        5. Only redirect clear essay-writing requests
      ...
      const response = await fetch("/api/query", {
        ...
        body: JSON.stringify({
          prompt: chatPrompt,
          inputText: input,
          maxTokens: 400 // Increased token limit
        }),
      });
      ...
      // Lighter response validation
      const isInvalidResponse = 
        aiResponse.length > 800 || // Higher character limit
        /\bhere('s| is) (a|your) (complete|full) essay\b/gi.test(aiResponse);
      ...
      setChatMessages(prev => [...prev, {
        sender: "ai",
        text: "I'm having trouble processing your request. Could you try again with a different question about your essay?"
      }]);
    } finally {
      setIsChatLoading(false);
      setChatInput("");
    ...
    <ThemeProvider attribute="class">
      <div className="flex flex-col flex-1 p-3 border rounded bg-white dark:bg-darker-custom">
        ...
        <div className="flex-1 border rounded p-2 overflow-auto bg-gray-50 dark:bg-blue-custom-dark mb-2 h-96">
          {chatMessages.length === 0 && (
            <div className="text-gray-400 italic p-2">
              Ask about your essay or other writing questions. Example: "How can I improve my thesis statement?"
            </div>
          )}
          {chatMessages.map((msg, idx) => (
            <div
        ...
        {/* Policy Reminder - softened language */}
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Note: This assistant provides essay analysis and writing advice, not complete essay writing services.
        </p>
      </div>
    </ThemeProvider>
  );
